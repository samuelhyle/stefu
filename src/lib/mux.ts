...
export function getMuxPlaybackUrl(playbackId: string | null): string | null {
  if (!playbackId) return null
  return `https://stream.mux.com/${playbackId}.m3u8`
}

export function getMuxThumbnailUrl(playbackId: string | null, time = 0): string | null {
  if (!playbackId) return null
  return `https://image.mux.com/${playbackId}/thumbnail.jpg?time=${time}`
}

export function getMuxAnimatedGifUrl(playbackId: string | null): string | null {
  if (!playbackId) return null
