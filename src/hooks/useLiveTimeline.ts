...
export function useLiveTimeline() {
  ...
  const [currentContent, setCurrentContent] = useState<typeof timelineContent[0] | null>(null)
  const [nextContent, setNextContent] = useState<typeof timelineContent[0] | null>(null)
  const [isTransitioning, setIsTransitioning] = useState(false)
  const [timeOfDay, setTimeOfDay] = useState('')
  const currentContentRef = useRef<typeof timelineContent[0] | null>(null)
  ...
  const findCurrentContent = useCallback((date: Date) => {
    const hours = date.getHours()
    const minutes = date.getMinutes()
    const currentTotalMinutes = hours * 60 + minutes
    
    let content: typeof timelineContent[0] | null = null
    let next: typeof timelineContent[0] | null = null
