...
interface EpisodesSectionProps {
  onVideoSelect?: (content: any) => void
}

export default function EpisodesSection({ onVideoSelect }: EpisodesSectionProps) {
  const [activeFilter, setActiveFilter] = useState('all')
  const [sortBy, setSortBy] = useState('popular')
  const [showSortMenu, setShowSortMenu] = useState(false)

  const filteredContent = useMemo(() => {
    let allContent: any[] = Object.entries(categoriesContent).flatMap(([, items]) => 
      items.map(item => ({ ...item, originalCategory: '' }))
    )
  ...
  const trendingContent = useMemo(() => {
    return [...featuredContent].sort((a: any, b: any) => (b.views || 0) - (a.views || 0)).slice(0, 5)
  }, [])
