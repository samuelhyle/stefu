  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (isSupabaseConfigured) {
        ...
        if (stored.find((s: { email: string }) => s.email === email)) {
          setStatus('error')
          setMessage('This email is already subscribed!')
          return
        }
  ...
  return (
    ...
    >
      <div className="flex items-start space-x-4 mb-6">
        ...
        <div>
          ...
          <p className="text-white/60">
            Get free basic access. Be the first to know about exclusive content,
            Sober Life Store drops, and events.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <input
            ...
            className="px-4 py-3 rounded-xl bg-obsidian-300/50 border border-white/10 text-white text-base placeholder-white/40 focus:outline-none focus:border-gold-500/50 transition-colors disabled:opacity-50"
          />
          <input
            type="email"
      ...
      <p className="text-white/30 text-xs mt-4">
        By joining, you agree to receive updates from Stefan Therman. Unsubscribe anytime.
      </p>
    </motion.div>
  )
}
