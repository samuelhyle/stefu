...
export default function SearchBar({ contents = [], onVideoSelect }) {
  ...
  const handleKeyDown = (e) => {
    const totalItems = results.length + recentSearches.filter(s => s.includes(query)).length
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault()
      const historyMatches = recentSearches.filter(s => s.includes(query) && query.trim())
