...
export default function RadioPlayer() {
  ...
  const widgetRef = useRef<ReturnType<Window['SC']['Widget']> | null>(null)
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (iframeRef.current) {
        ...
        widgetRef.current.bind(window.SC.Widget.Events.PLAY_PROGRESS, (data: unknown) => {
          const d = data as { currentTrack?: { title?: string } }
          if (d.currentTrack) {
            setTrackTitle(d.currentTrack.title || 'Stefu Radio')
          }
        })
      }
    }
